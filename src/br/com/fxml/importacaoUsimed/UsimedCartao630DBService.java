 ...
 * @author joao.santos
 */
public class UsimedCartao630DBService implements UsimedCartao630Services{

    final String inserir = "INSERT INTO cartao630(descricao,t,data,credito,debito,origem) VALUES(?,?,?,?,?,?)";
    final String atualizar = "UPDATE cartao630 SET descricao = ?,  t = ?, data = ?, credito = ?, debito = ?, origem = ? WHERE id = ?";
    final String buscar_todos = "SELECT id, descricao, t, data, credito, debito, origem FROM cartao630";
    final String apagar = "DELETE FROM cartao630 WHERE id = ?";
    ...
    public List<UsimedCartao630DADOS> buscarTodas() {
        List<UsimedCartao630DADOS> dados = new ArrayList<>();
        try {
            ...
            while (rs.next()) {
                UsimedCartao630DADOS dado = extraiConta(rs);
                dados.add(dado);
            }
        } catch (SQLException ex) {
            Alert alerta = new Alert(Alert.AlertType.ERROR);
    ...
    private UsimedCartao630DADOS extraiConta(ResultSet resultadoBusca) throws SQLException {
        UsimedCartao630DADOS dados = new UsimedCartao630DADOS();
